<div class="container-fluid" id="info">
    <div class="row">
        <div class="col-12 mb-5">
            ...
            <div class="row">
                <div class="col-6 m-1">
                    <form wire:submit.prevent="editProduct" id="productForm">
                        ...
                        <div class="row">
                            ...
                            <div class=" col-12 p-3 mb-5  border rounded " id="name-desc">
                                <div class="col-12 mb-3">
                                    <input class="form-control rounded" placeholder="نام محصول"
                                        wire:model.defer="productEditable.title" value="{{ $productEditable['title'] ?? '' }}" />

                                    @error('productEditable.title')
                            ...
                            <div class=" col-12 p-3 mb-5 border rounded" id="name-desc">
                                <div class="col-12">
                                    <select class="form-control  rounded" wire:model.defer="productEditable.category"
                                        id="categories">
                                        @foreach (\App\Models\Category::select(['id', 'title'])->get() as $category)
                                            <option value="{{ $category->id }}"
                                                wire:click="$set('productEditable.category',$event.target.value)"
                                                :key="{{ $category->id }}">{{ $category->title }}
                                            </option>
                                        @endforeach
                                    </select>

                                    @error('productEditable.category')
                                        <span class="text text-danger"> {{ $message }} </span>
                                    @enderror
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-6 d-flex align-items-center">
                                        <input id="price" class="form-control rounded" placeholder="قیمت "
                                            wire:model.defer="productEditable.price" value="{{ $productEditable['price'] ?? '' }}" />
                                        <label for="price" class="m-1"> تومان </label>


                                        @error('productEditable.quantity')
                                            <span class="text text-danger"> {{ $message }} </span>
                                        @enderror
                                    </div>

                                    <div class="col-6 d-flex align-items-center">
                                        <input class="form-control rounded" wire:model.defer="productEditable.quantity"
                                            placeholder="موجودی" value="{{ $productEditable['quantity'] ?? '' }}" />
                                    ...
                                    <select class="form-control  rounded" id="categories">
                                        @foreach (\App\Models\Vendor::select(['id', 'vendor_name'])->get() as $vendor)
                                            <option value="{{ $vendor->id }}"
                                                wire:click="$set('productEditable.vendor_id',$event.target.value)"
                                                :key="{{ $vendor->id }}">{{ $vendor->vendor_name }}
                                            </option>
                                        @endforeach
                                    </select>

                                    @error('productEditable.vendor_id')
                                        <span class="text text-danger"> {{ $message }} </span>
                                    @enderror
                                </div>
                            </div>
